# Quartly Adventure Game – Java OOP & Data Structures

## Overview

This project implements a **text-based adventure game** in Java, focusing on **data structures, cloning, and iterators**. It extends previous assignments by introducing a new custom data structure — `QuartlyLinkedList` — which connects rooms in four directions (`north`, `east`, `south`, `west`) using `QuartNode` elements.
...
1. Open the project in **IntelliJ IDEA** (Java 9.0.4 or higher).
2. Compile all files under the `src` folder.
3. Run the `Main` class to start the test suite or the adventure simulation.
